...
Dispatch only `agents/executor.md`. It receives issue URL when present, plan revision, task ID, base SHA, allowed files, acceptance criteria, verification, and evidence destination. It posts `templates/issue-task-evidence.md` in issue mode or the same eight-field envelope in direct mode. A `BLOCKED` outcome stops: issue mode uses `stage:blocked` + `needs-human`; direct mode appends `Resume: <phase/task>` with no GitHub state.
