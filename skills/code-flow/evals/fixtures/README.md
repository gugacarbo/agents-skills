...
| E4   | #45   | `code-flow:active + stage:needs-delivery-review`                   | `e4-self-authorship.json` (mock com run_id def-456 em issue-writer + executor) |
...
`dev/tests.sh`) apontando para os mocks JSON aqui. O `run-evals.mjs` não inclui
