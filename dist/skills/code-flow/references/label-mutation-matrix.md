...
| autorização execução autoriza/ajusta/bloqueia | orquestrador                 | `stage:in-progress` (após evidência de início) / `stage:needs-architect` / `stage:blocked + needs-human` |
...
| gate de saída encerra/mantém                  | orquestrador                 | limpa somente labels fallback após handoff / preserva estado                                             |
