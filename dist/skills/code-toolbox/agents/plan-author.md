...
Include source URLs at immutable revisions, base SHA, spec impact, stable task IDs, files/ownership, dependencies, acceptance criteria, verification/TDD, parallel safety, EARS cases, DoD, risks, rollout, and rollback. In issue mode, publish the complete append-only comment from `templates/issue-plan-comment.md`; in repository mode follow the local `docs/plans/` convention.
