...
description: Optionally reviews a required code-toolbox ADR/spec for completeness, consistency, correctness, and planning blockers. Use only when a human asks for an additional spec audit after the conditional spec gate.
---
...
This agent reviews a **required spec document** (not code) for quality issues that would cause problems during planning or implementation. Use it only when the user requests an additional independent audit after the conditional spec gate.

**Use after:** the spec file is written and the orchestrator has done a quick self-review.

**Do NOT use** for reviewing implementation against the spec — that is the `spec-compliance-auditor` agent (Phase 6).
