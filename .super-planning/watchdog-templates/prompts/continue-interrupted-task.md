# Continue an interrupted super-planning task

Use this prompt only for a thread-bound continuation heartbeat.

1. Read the active `docs/jobs/<plan-id>/super-plan.json`, its
   `progress-ledger.md`, relevant task reports and logs, and the current Git
   state.
...
5. Otherwise, resume the next safe action through the normal
   super-planning lifecycle. Respect dependencies, the recorded worktree
