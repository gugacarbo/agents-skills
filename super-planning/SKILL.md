...
- **Sequential mode:** one implementer + reviewer dispatch according to `reviewCadence`. With `per_task`, dispatch the reviewer as soon as the task finishes; with `per_batch`, dispatch one reviewer for the whole batch; with `final_only`, defer review to Phase 7 but still dispatch one reviewer per batch there. Best for dependent tasks or overlapping files.
- **Parallel mode:** dispatch 2–4 subagents simultaneously. Review timing is controlled by `reviewCadence`; with `per_task`, each finished task is reviewed immediately; with `per_batch`, review starts once the whole batch is ready; with `final_only`, review is deferred to Phase 7 but still happens one reviewer per batch. Requires file-level isolation.
...
- **User confirmation of discovered profiles:** after auto-discovery, the orchestrator must show the discovered agents/models plus its recommendation and ask the user what to do next. When the platform exposes a structured ask/confirm/question tool, including `request_user_input` / a request user input tool, and the current mode/session allows using it, that tool is required; plain text is the fallback when the tool does not exist or is not callable in the current mode/session.
