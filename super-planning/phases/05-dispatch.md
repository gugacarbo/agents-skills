...
10. If `reviewCadence=per_task`, set each finished task to `reviewing` via script, then dispatch a reviewer subagent for each task in the same response (or the next response) after all implementers return. Do not wait for sibling tasks in the same parallel batch to be reviewed before starting the next reviewer.
