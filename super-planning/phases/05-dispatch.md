...
For a human-readable task brief (useful when the implementer needs a self-contained markdown summary of their task), generate it with [`scripts/render-task-md.sh`](../scripts/render-task-md.sh):

```bash
bash super-planning/scripts/render-task-md.sh \
  --input docs/jobs/NNNN-<feature-name>/super-plan.json \
  --task-id Task-A-1 \
  --output docs/jobs/NNNN-<feature-name>/Task-A-1/task-brief.md
```

## Report File Convention

Every implementer is expected to produce a full report in the task directory (materialized before dispatch). Default convention:

```
docs/jobs/{NNNN-<feature-name>}/{task-id}/report.md
```

Example for task `Task-A-1` under plan `0003-auth-middleware`:

```
docs/jobs/0003-auth-middleware/Task-A-1/report.md
```
...
Then the orchestrator uses that script's `materialize-task-logger` subcommand to generate thin task-local wrappers **before dispatching implementers**:

- `docs/jobs/{NNNN-<feature-name>}/{task-id}/`
- `docs/jobs/{NNNN-<feature-name>}/{task-id}/log-task.sh`
- `docs/jobs/{NNNN-<feature-name>}/{task-id}/progress.log` (empty, ready for append-only writes)

This ensures implementers can log `started`/`ready_for_review` events during execution. The remaining task artifacts (`report.md`, `review-package.diff.md`) are still materialized in Phase 6.

## What Phase 5 Must Not Create

Phase 5 does **not** create:

- task-local `report.md`
- task-local `review-package.diff.md`

The ledger already exists by this phase and must be regenerated by the active `super-plan.sh` helper path whenever the registry changes. Report files and review packages are still materialized in Phase 6.

## Review Package Convention

Before dispatching a reviewer, Phase 6 generates a review package with the
active helper [`scripts/review-package.sh`](../scripts/review-package.sh).
Record the current commit as the task's `baseCommit` before dispatching the
implementer. The package must use that recorded base, never `HEAD~1`, because
...
The package is written in the task directory once Phase 6 has materialized it:
...
The same base commit is reused for a re-review after fixes. A final whole
branch package uses `git merge-base <base-branch> HEAD` as its base.
...
**Critical:** In parallel mode with `reviewCadence=per_task`, implementation and review overlap across responses. The orchestrator must launch reviewers for all finished tasks in the response immediately following the implementer batch return, rather than waiting for the rest of the batch.
...
> **Caveat:** Worktree isolation is an opencode-specific dispatch parameter. If using a different platform, ensure equivalent isolation (separate working directories per subagent).
...
3. Use the task `layer` field to describe whether the work is `foundation`, `core`, `surface`, or `final`.
...
- **File scope:** If a task declares specific files, the resulting commit should not touch files outside that list. Infrastructure files (`package.json`, lockfiles, config files) may be modified ONLY if the task explicitly requires it OR if the change is purely additive (adding a dependency). Structural changes to infra files require orchestrator approval.
