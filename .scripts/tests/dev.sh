...
test_dev_rebuilds_after_a_source_change() {
  ...
  prompt_input="$tmp/prompt-input"
  target="$tmp/published-skills"
  mkdir -p "$project/.scripts" "$project/skills/example-skill"
  cp "$REPO_ROOT/.scripts/build.sh" "$project/.scripts/build.sh"
  cp "$REPO_ROOT/.scripts/dev.sh" "$project/.scripts/dev.sh"
  chmod +x "$project/.scripts/build.sh"
  printf '%s\n' '---' 'name: example-skill' 'version: initial' '---' >"$project/skills/example-skill/SKILL.md"
  printf '%s\n' "$target" >"$prompt_input"

  env AGENTS_SKILLS_PROMPT_INPUT="$prompt_input" sh "$project/.scripts/dev.sh" >"$tmp/output.log" 2>&1 &
  dev_pid=$!
