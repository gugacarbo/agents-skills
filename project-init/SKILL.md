...
/project-init base-only              # Scaffold with `_base` only (reserved alias, no family/derivative)
...
2. **Instructions** — each layer's `AGENTS.md` (and `REQUIREMENTS.md`) is read in cascade order and merged. Instructions are grouped by *concern* using the section headings in those files (e.g., `## Setup`, `## Framework init`, `## Package manager`, `## Test runner`, `## Optional Tools`). For each concern:
...
3. **Select template** — if an argument was provided, resolve it against the template tree. `base-only` is a reserved alias that resolves to `_base` only; it is not a folder under `templates/`. Otherwise, if the argument does not match any family or derivative, list the available templates and re-ask interactively instead of failing. If a family is selected without a derivative (e.g., `typescript`), the resolved layer stack is `_base` → family. If a derivative is selected (e.g., `typescript/vite`), the resolved layer stack is `_base` → `typescript` → `typescript/vite`. The agent always resolves and applies the full stack — see Cascade Principle.
...
8. **Ask about optional tools** — read the "Optional Tools" tables from every resolved layer's `AGENTS.md` in cascade order and merge them by **tool id** (the `Tool` column). A deeper layer may override the install command or description for a tool already defined by a shallower layer, or add a brand-new tool; tools omitted by deeper layers remain inherited. Do not offer a tool that the resolved stack already makes mandatory through its default dependencies or framework choice. Use a multi-select question tool with none pre-selected to let the user pick which tools to include. Briefly explain what each tool does. Only add selected tools to the setup instructions. Do not hardcode the list here — always read it from the template files to avoid drift.
9. **Resolve setup instructions in cascade** — read the `AGENTS.md` (and `REQUIREMENTS.md` where present) of every resolved layer in cascade order. For each concern section (`## Setup`, `## Framework init`, `## Package manager`, `## Dependencies`, `## Test runner`, etc.), keep only the instruction from the **deepest layer that addresses that concern**; discard shallower layers' instructions for that same concern. Accumulate concerns that no deeper layer overrides. The result is a single, deduplicated, conflict-free instruction set.
10. **Print setup instructions** — output the resolved instruction set from step 9, in the order the deepest layer that owns the first concern documents it. Concretely this typically includes:
    (a) CASA Standard / docs-router init (from `_base` unless overridden). If the CASA installer would overwrite the just-scaffolded `AGENTS.md`, warn the user and suggest running CASA first, then re-scaffolding the template `AGENTS.md` on top.
    (b) Framework/package init — exactly **one** CLI command for the framework-init concern, sourced from the deepest layer that addresses it (e.g., for `typescript/vite` use the Vite CLI, not a TypeScript CLI followed by Vite). **Important**: some framework init CLIs (e.g., `pnpm create vite .`, `bun init`) require an empty target directory. If the scaffolded convention files would block the CLI, instruct the user to: (1) move the scaffolded files aside temporarily, (2) run the framework CLI, (3) restore the scaffolded files (deepest-wins for any path conflicts). Convention files are designed to layer on top of generated project files without conflicts.
    (c) Optional tool installs — for each tool selected in step 8, use the install command from the merged optional-tools registry after cascade resolution. Substitute the `<pm>` placeholder with the resolved package manager.
...
- `_base/` — applied to every project; defines shared conventions, the default "Optional Tools" registry, and the post-scaffold setup steps. Read its `AGENTS.md` for the authoritative list of files and behaviors it contributes.
- `<family>/` — a runtime/ecosystem base (e.g., `typescript`, `bun`). Each has `AGENTS.md` and usually `REQUIREMENTS.md`. Families without derivatives (e.g., `bun`) are valid — the resolved layer stack is `_base` → family.
...
Template authors should organize each `AGENTS.md`/`REQUIREMENTS.md` by **concern** using clearly labeled sections. The standard sections are:

- `## Runtime` — runtime environment
- `## Setup` — first-run or post-scaffold setup guidance
- `## General` — project-wide conventions not tied to a specific runtime
- `## Code Style` — formatting/editor expectations
- `## Git` — branch, commit, and ignore conventions
- `## CI/CD` — default CI expectations
- `## Agent Behavior` — interaction and decision-making constraints for agents
- `## Build tool` — build/dev server selection when applicable
- `## Package manager` — package manager choice
- `## Module system` — ESM/CJS configuration
- `## Entry point` — application entry file
- `## Test runner` — test framework and commands
- `## Lint & Format` — linter and formatter
- `## Type checking` — type check command
- `## Dead code` — dead code detection tool
- `## Git hooks` — pre-commit/pre-push hooks
- `## Framework init` — the CLI command to initialize the project
- `## Dependencies` — install commands for required packages
- `## Optional Tools` — user-selectable additional tools
- `## Project Structure` — intended top-level layout
- `## Skills (opencode)` — recommended companion skills for the repo

The cascade resolver keys off these sections; a deeper layer overrides a shallower layer only for concerns it explicitly addresses. Sections present only in shallower layers (and not overridden deeper) are kept as-is.
...
- Do NOT treat `base-only` as a real template directory. It is a reserved alias for resolving `_base` only.
